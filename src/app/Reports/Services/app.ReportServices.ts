...
export class ReportService {
    ...
    public GetAllMemberDetailsReport() {
        var apiUrl = "	http://localhost:54354/api/MemberDetailsReport/";
    ...
    public GetYearWiseReport(year: YearwiseRequestModel) {
        var apiUrl = "	http://localhost:54354/api/YearwiseReport/";
        let headers = new HttpHeaders({ 'Content-Type': 'application/json' });
        headers = headers.append('Authorization', 'Bearer ' + `${this.token}`);
    ...
    public GetMonthWiseReport(year: YearwiseRequestModel) {
        var apiUrl = "	http://localhost:54354/api/MonthwiseReport/";
        let headers = new HttpHeaders({ 'Content-Type': 'application/json' });
        headers = headers.append('Authorization', 'Bearer ' + `${this.token}`);
    ...
    public GetRenewalReport(year: RenewalRequestModel) {
        var apiUrl = "	http://localhost:54354/api/RenewalReport/";
        let headers = new HttpHeaders({ 'Content-Type': 'application/json' });
        headers = headers.append('Authorization', 'Bearer ' + `${this.token}`);
